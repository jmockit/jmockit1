 ...
 * For a mock <em>field</em>, the test itself can provide the instance by declaring the field as <tt>final</tt> and
 * assigning it the desired instance (or <tt>null</tt>).
 * <p/>
 ...
 * the mocked type, except for non-eligible ones (primitive wrappers, <tt>String</tt>, and collections/maps).
 ...
 * class/interface "<code>A</code>" has return type <tt>A</tt>, then it will return itself by default).
 * Finally, when new cascaded instances are created, {@linkplain Injectable @Injectable} semantics apply.
 * <p/>
 * Static <em>class initializers</em> (including assignments to <tt>static</tt> fields) of a mocked class are not
...
public @interface Mocked
{
    ...
    * (Note that <tt>static final</tt> fields initialized with <em>compile-time</em> constants are not assigned at
    ...
    * static initialization, potentially leaving static fields <tt>null</tt> and later causing
    * <tt>NullPointerException</tt>'s to occur.
    */
