 ...
 * inject <em>non-mocked</em> (real) instances when applied as <code>@Tested(fullyInitialized = true)</code>.
 * Non-mocked instances are either automatically created or obtained from other <code>@Tested</code> fields.
 * <p>
 * The {@link mockit.Expectations} class provides an API for the <em>record-replay</em> model of recording expected invocations which are
 ...
 * The {@link mockit.Verifications} class extends the record-replay model to a <em>record-replay-verify</em> model, where expectations that
 * were not recorded can be verified explicitly <em>after</em> exercising the code under test (ie, after the replay phase).
 ...
 * <p>
 * {@linkplain mockit.MockUp <code>MockUp&lt;T></code>} is a generic base class (where <code>T</code> is the type to be faked) which allows the
 * definition of fake implementations for external classes.
 * Methods in a fake class that take the place of the "real" methods in class <code>T</code> are indicated with the
 ...
 * For the <em>Faking</em> API, see the "<a href="http://jmockit.github.io/tutorial/Faking.html" target="tutorial">Faking</a>" chapter.
 */
package mockit;
