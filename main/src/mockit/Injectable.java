 ...
 * or <em>injected</em> into the code under test.
 * Such instances can be said to be proper <em>mock objects</em>, in contrast to the mocked instances of a regular <code>@Mocked</code> type.
 * <p>
 * When the type of the injectable is <code>String</code>, a primitive wrapper, a {@linkplain Number number type}, or an enum, it is
 * <em>not</em> mocked.
 * A non-empty {@link #value} must then be provided, except in the first case where the empty string is used by default.
 * <p>
 * For the duration of each test where the mock field/parameter is in scope, <em>only one</em> injectable instance is mocked; other
 * instances of the same mocked type are not affected.
 ...
 * <p>
 * When used in combination with {@linkplain Tested @Tested}, the values of injectable fields and parameters will be used for automatic
 * injection into the tested object.
 * Additionally, this annotation can be applied to non-mocked fields of primitive or array types, which will also be used for injection.
 *
 * @see #value
